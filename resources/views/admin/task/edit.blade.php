...
@section('content')
    <div class="container">
        <div class="d-flex justify-content-between mb-4">
            <h4>Edit Task</h4>
            <a href="{{ route('admin.task.index') }}" class="btn btn-success">Back</a>
        </div>
    </div>

    <div class="container">
        <div class="card">
            <form class="m-3" action="{{ route("admin.task.update", $task->id) }}" method="POST">
                @csrf
                @method("PUT")
                <div class="m-3">
                ...
                <div class="m-3">
                    <label for="name" class="fw-bold" >Owner: </label>
                    <p class="lead">{{$task->user->name}}</p>
                </div>
